...
A draft RDR is named `{timestamp}-{slug}.md`; a merged one is `{timestamp}-{slug}.done.md` — the filename is the state, no in-file marker to parse, and the suffix matches how `/auto-action` marks a finished plan.
...
2. In `.context/rdr/`, find draft RDR files (no `.done.md` suffix) whose filename ends in `-{slug}.md`. If none, tell the user there's nothing to merge and stop. If more than one, list them and ask the user which to merge.
...
4. Rename the RDR file in place from `{timestamp}-{slug}.md` to `{timestamp}-{slug}.done.md` — keep it as permanent decision history, never delete it.
5. Tell the user the committed spec path and the RDR's new filename.

Completion criterion: the matching RDR file is renamed to `.done.md`, and `.context/req/{slug}.md` holds its content.
