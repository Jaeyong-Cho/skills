...
Folds a draft RDR from `.context/req/rdr/` — the record `/req` produced and `/archi`/`/planning`/`/auto-action` built against — into its committed spec, `.context/req/{slug}.md`, now that implementation is finished.
