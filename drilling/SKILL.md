...
**Recording dependencies:** once every branch is built out to atomic leaves, scan all of them for real dependencies — a leaf that reads state, calls a function, or uses a component another leaf produces can't be executed before that other leaf is done, even across branches (a resize-related leaf disabling itself based on a collapse flag depends on the leaf that creates that flag). Annotate the dependent leaf inline: `(after: {path})`, or `(after: {path1}, {path2})` for more than one. This is the only ordering the tree records — sibling position never implies order, but a recorded dependency does and has to be respected. Leaves with no dependency stay unordered; that's still the user's call.
