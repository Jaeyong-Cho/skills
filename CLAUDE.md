...
- Search: before reading any file under `~/wiki` or a project's `~/wiki/projects/{slug}/wiki/` directly, run `qmd query` first per `references/qmd-search.md` — don't grep/read/walk the `index.md` chains until it returns nothing.
