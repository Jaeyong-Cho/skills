...
4. **Mark Closeout.** If every hole passed review and every test passed, mark `[x] Review + Test` in the plan's `## Closeout` checklist, then rename the plan file from `{timestamp}-{slug}.md` to `{timestamp}-{slug}.done.md` — that rename is what the already-done check above looks for on a future invocation. This happens regardless of the user's commit decision in step 3; the checklist item and the rename are about review and tests, not the commit. Leave both the checklist and the filename unchanged if anything failed, and skip **Merge RDR/ADR** below entirely. If both are clean, run **Merge RDR/ADR** below.
...
Runs only after the plan file has just been renamed to `.done.md` above (Full Execution, or Self-Plan Review & Test with everything clean) — never after Self-Plan Write, since holes mean implementation isn't finished yet.

Take `{slug}` from the plan's own filename.

If a draft RDR (`{timestamp}-{slug}.md`, no `.done.md` suffix) exists in `.context/rdr/`:
...
2. Rename the RDR file in place to `{timestamp}-{slug}.done.md` — keep it as permanent decision history, never delete it.

If a draft ADR (`{timestamp}-{slug}.md`, no `.done.md` suffix) exists in `.context/adr/`:
...
2. Rename the ADR file in place to `{timestamp}-{slug}.done.md` — keep it as permanent decision history, never delete it.

If neither draft exists for this slug, there's nothing to merge — skip silently.

Completion criterion: any draft RDR/ADR found for this slug is renamed to `.done.md`; `.context/req/{slug}.md` holds the RDR's content if one existed; `.context/archi/{slug}.md` reflects the implemented Static/Dynamic View if an ADR existed.

## When Done

**Already Done:** "Auto-action: this plan is already complete." followed by the plan file's `.done.md` name. Do nothing else.

**Full Execution:** "Auto-action complete." followed by the per-step summary, then the **Merge RDR/ADR** outcome: if a draft RDR was found, report the committed spec path (`.context/req/{slug}.md`) and the RDR's new `.done.md` filename; if a draft ADR was found, report the committed ADR path, its new filename, and the architecture doc path (`.context/archi/{slug}.md`). If any step failed, skip **Merge RDR/ADR** and this reporting entirely — leave both drafts in place so nothing is committed for unfinished work.
