...
## Reading `~/wiki` or `spec/` (progressive disclosure)

**Frontmatter first, always** — read it before the body, on every `.md` file under `~/wiki` or a target repo's `spec/`, even a file already named/known (opened directly, not found via search). It's a few lines that confirm `type`/relevance/`timestamp`-freshness before paying for the rest of the file; skip the body read entirely if the frontmatter alone answers the question.

1. Walk the `index.md` chain down from the relevant root (`journal/`, `research/`, `roadmap/`, or `spec/`) toward the likely date/project/EPIC — each `index.md` narrows which subdirectory to enter next.
...
This is the whole point of carrying frontmatter: a read should resolve from frontmatter alone whenever it can, and only pay for the full body on an actual candidate.
