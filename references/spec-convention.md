...
- `spec/{epic-slug}/index.md` follows `../template/spec-epic-index.md` — a Business value line plus the EPIC's intro, and a list linking to its STORY files.
- Each STORY file follows `../template/spec.md`'s format — a Value-to-user line (what the user can do, or gets, once this ships) plus the spec lines, ending in an AC (acceptance criteria) block.
