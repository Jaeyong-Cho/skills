...
- `spec/{epic-slug}/index.md` follows `../template/spec-epic-index.md` — a Business value line and a Completion criteria line (the observable state that proves the EPIC is done) plus the EPIC's intro, and a list linking to its STORY files.
- Each STORY file follows `../template/spec.md`'s format — a Value-to-user line (what the user can do, or gets, once this ships) and a Completion criteria line, plus the spec lines, ending in an AC (acceptance criteria) table, one row per criterion, per `requirement-engineering.md`. Completion criteria is the coarse, checkable-now line; AC is the detailed Given/When/Then breakdown, as many rows as the STORY needs, filled in when the STORY is picked up.
