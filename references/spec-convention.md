...
- Each STORY file follows `../template/spec.md`'s format — a Value-to-user line (what the user can do, or gets, once this ships) and a Completion criteria line, plus the spec lines, ending in an AC (acceptance criteria) block. Completion criteria is the coarse, checkable-now line; AC is the detailed Given/When/Then breakdown filled in when the STORY is picked up.
