...
`verify/run.sh` and every domain `run.sh` are executable. A domain runner exits non-zero when its domain fails and zero when it passes. The root runner may provide `fast` and `full` modes, but the loop interface selects a maximum functional level:
...
The functional chain is ordered: IT starts only after UT passes, and E2E starts only after IT passes. Style and Architecture are independent of that chain and of each other; they may run in parallel. A failure prevents later functional levels from starting. Already-running independent checks may finish so their evidence is retained.

The root runner should preserve the same semantics when it exposes modes: fast verification is normally UT + Style + Architecture; full verification includes the selected project-native IT and E2E checks.
...
- `V-DIR-002`: `verify/` has `ut/`, `it/`, `e2e/`, `style/`, and `archi/`.
- `V-RUN-001`: `verify/` has executable `run.sh`.
- `V-RUN-002`: every top-level domain has executable `run.sh`.
