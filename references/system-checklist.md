...
- Target platform(s) — claude / copilot / pi, and this goal's chosen sub-agent mechanic per platform: Claude's Task tool, Copilot's `--agent`, or pi's `pi-subagents` package per `pi-custom-subagent.md` — anything not covered by that reference (Claude, Copilot, or a pi-subagents field the reference doesn't mention) is found by dispatching a sub-agent to check the real installed tool (`--help`, its docs), never assumed from memory
