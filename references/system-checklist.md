...
- Target platform(s) — claude / copilot / pi, and this goal's chosen sub-agent mechanic per platform: Claude's Task tool, Copilot's `--agent`, or Pi's `pi-subagents` package per `pi-custom-subagent.md`; shared skills say only “dispatch a sub-agent,” while platform-specific syntax belongs in the host's reference — anything not covered by that reference is found by checking the real installed tool (`--help`, its docs), never assumed from memory
