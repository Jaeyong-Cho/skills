...
Save: `.pf/reports/YYYY-MM-DD-<slug>.html` (slug = feature name, lowercase, hyphens, max 40 chars)
...
**Always include:**
- **Behavior results** — table: behavior, layer (`[value]`/`[aspect]`/`[object]`), result (✅ GREEN / ❌ still RED), test name
- **Pass rate** — progress bar: behaviors green / total
- **Layer map** — which files own which layer; node-edge diagram (`file:line` → layer label)

**Include when content warrants:**
- **Refactor summary** — what changed during refactor step; before/after diagram
- **Design decisions** — key VAO decisions from grill session (condensed, not transcript)
- **Open questions** — unresolved concerns or follow-up items surfaced during impl

Footer: `Generated by pf-grill-impl · date · slug`

Print path after saving:
```
Report: .pf/reports/YYYY-MM-DD-<slug>.html
```
