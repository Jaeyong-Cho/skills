...
Return up to three prioritized findings in descending impact or risk order. If no in-scope finding remains, say so plainly, write `Violation: None`, and identify the evidence supporting that conclusion. For a finding, identify the exact violated checklist item or criterion from the linked review skill. Use this contract:
