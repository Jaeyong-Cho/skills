...
7. **Present the full tree**, indented by level with the axis used at each level labeled inline (e.g. `by phase:`, `by component:`). Number top-level children in presentation order (`1`, `2`, …) and every leaf beneath each with a dotted id in traversal order (`1.1`, `1.2`, … under section 1), so leaves stay addressable outside the tree. Then the execution order from step 6 as a flat list of dotted ids, grouping parallelizable leaves under the same round number, followed by the critical path: the chain of ids it runs through and which one is most exposed to slipping.
