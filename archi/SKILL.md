...
`mkdir -p .context/adr` if needed. Tell the user the file path, and that it's a draft ADR that `/merge-archi` will fold into `.context/adr/{slug}.md` once implementation completes. If the written ADR runs over the template's word/sentence budget, say so and suggest running `/drilling` to split the goal further — but don't block on it. Next step: `/planning`.
