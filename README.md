...
| `/roadmap` | `~/wiki/roadmap/{project}/{open,in-progress,done}/{epic-slug}/{story-slug}/`, `~/wiki/roadmap/archive/{project}/` | Manage a project's EPIC/STORY/Task roadmap with the user however it comes up — deadlines, breakdown, fixes, a cross-project "today's work" report, archiving — the only fixed rule is state = directory (open/in-progress/done), moved with `mv`; a project may span multiple repos or none, independent of `/to-plan`/`/do-plan` and any repo's `spec/` |
