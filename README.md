...
| pi coding agent | Installs `ponytail`'s skill commands via [skills.sh](https://www.skills.sh/) and the global `pi-subagents` package (`pi install npm:pi-subagents`) |
...
All skills above are user-invoked, except `/experiment`, `/d-handoff`, `/to-plan`, `/do-plan`, and `coding-interview`, which the agent can also fire on its own — `/experiment` when a question needs something actually run to get evidence; `/d-handoff` when the user wants to wrap up the day, or when `/end-of-day` reaches for it before drafting; `/to-plan` and `/do-plan` when a solution plan needs to be executed; `coding-interview` when the user says "interview me"/"study mode"/pastes code for interview practice, without needing to name it. Most artifacts land in a skill-named directory at the project root; `/end-of-day`, `/d-handoff`, `/to-plan`, and `/experiment` instead read and write the global `~/wiki/` layout documented in `CLAUDE.md`'s Context Structure section (journal/research/handoff), since they're personal daily-workflow tools rather than per-project outputs.
