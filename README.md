...
All skills above are user-invoked, except `/experiment`, `/d-handoff`, `/to-plan`, and `/do-plan`, which the agent can also fire on its own — `/experiment` when a question needs something actually run to get evidence; `/d-handoff` when the user wants to wrap up the day, or when `/end-of-day` reaches for it before drafting; `/to-plan` and `/do-plan` when `/execute-solution` reaches them to turn a picked solution into a real change. Most artifacts land in a skill-named directory at the project root; `/end-of-day`, `/d-handoff`, `/to-plan`, and `/experiment` instead read and write the global `~/wiki/` layout documented in `CLAUDE.md`'s Context Structure section (journal/research/handoff), since they're personal daily-workflow tools rather than per-project outputs.
