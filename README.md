...
| `/workflow` | `spec/**`, one review-plan per SEQ in `.context/inbox/plan/` | Run a goal straight through `/spec`'s scen → req → cmp → seq stages, then fan out one subagent per resulting SEQ to run `/co-plan` scoped to just that sequence |
