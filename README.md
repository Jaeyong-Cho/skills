...
| `/find-solutions` | `./solutions/nn-{slug}.md` (or `.../NN-{slug}/solutions/nn-{slug}.md`) | For each `problem-NN.md`, dispatch sub-agents (reuse-check first, then alternatives; `/experiment` where reading can't settle feasibility) to find one or more options that reduce the Gap, compare them against the problem's evaluation criteria/constraints, and recommend one |
