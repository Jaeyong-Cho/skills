...
| `/story-grill-me` | — | Run `/grill-me` to build an agile Story in one pass: persona, user value, trigger, happy/alternate/edge/negative scenarios, dependencies, INVEST check |
| `/gtd-grill-me` | — | Run `/grill-me` to clarify a brain-dump using GTD: capture, then for each item decide actionable or not, single action or project, do-now/delegate/defer/calendar |
| `/to-gtd` | `~/wiki/gtd/{next-actions,waiting-for,someday-maybe,calendar,reference}.md`, `~/wiki/gtd/projects/{project-slug}.md`, `~/wiki/gtd/archive/{yyyy}/{mm}/` | File a `/gtd-grill-me` session into the GTD system, and keep it live afterward — check items off, move completed items/projects to the dated archive; lints the tree after every write |
| `/gtd-review` | `~/wiki/gtd/**` (via `/to-gtd`) | GTD Weekly Review: sweep for stalled projects, overdue Waiting For, past-due Calendar, and Someday/Maybe, grill the user on each, then file the decisions with `/to-gtd` |
