...
| `/intent-grill-me` | `./intents.md` (or `~/wiki/today/research/NN-{slug}/intents.md`) | Run `/grill-me` to uncover the real intent behind a request — motivation, need, pain/friction, desired outcome, constraints, assumptions — before any current-state, problem, or solution work starts; writes an OKF-frontmatter document |
| `/define-problem` | `./problems/nn-{slug}.md` (or `.../NN-{slug}/problems/nn-{slug}.md`) | Given an `intents.md`, dispatch sub-agents (and `/experiment` where reading alone can't settle it) to find the real current state, diff it against the desired outcome for the gap, and set evaluation criteria/boundary per resulting problem — never asks the user for a fact |
| `/find-solutions` | `./solutions/nn-{slug}.md` (or `.../NN-{slug}/solutions/nn-{slug}.md`) | For each `problem-NN.md`, dispatch sub-agents (reuse-check first, then local alternatives, then a cited web reference example; `/experiment` where reading can't settle feasibility) to find one or more options that reduce the Gap, compare them against the problem's evaluation criteria/constraints, and recommend one |
| `/execute-solution` | `{plan-file}.md`, `{plan-file}.report.md` | Feed a `/find-solutions` Decision and its problem's evaluation criteria into `/to-plan` (already-grilled input, no re-interview) then `/do-plan` to build, verify, and commit it for real; updates the solution/problem Status once done |
| `/evaluate-solution` | `./evaluations/nn-{slug}.md` (or `.../NN-{slug}/evaluations/nn-{slug}.md`) | After `/execute-solution`, dispatch a sub-agent to re-check the real current state against the problem's Desired state (not just trust the report's green tests), verdict Resolved/Partially Resolved/Unresolved, name any remaining gap, and write feedback recommending the next cycle or none |
...
| `/l1-grill-me` | — | Run `/grill-me` to nail down one L1 orchestration flow's step sequence: trigger, ordered L2/L3 calls, branches, end state — feeds `/l1-implement` (via `/to-code`) |
| `/l2-grill-me` | — | Run `/grill-me` to nail down one L2 business rule: domain input, decision logic/branches, L3 interface dependency (if any), output — feeds `/l2-implement` (via `/to-code`) |
| `/l3-grill-me` | — | Run `/grill-me` to nail down one L3 technical operation: interface, mechanism, failure modes — feeds `/l3-implement` (via `/to-code`) |
| `/to-code` | — | Turn a finished `/l1-grill-me`/`/l2-grill-me`/`/l3-grill-me` session directly into code by invoking the matching `/l1-implement`/`/l2-implement`/`/l3-implement` — no plan file |
...
All skills above are user-invoked, except `/experiment`, `/d-handoff`, `/to-plan`, `/do-plan`, and `coding-interview`, which the agent can also fire on its own — `/experiment` when a question needs something actually run to get evidence; `/d-handoff` when the user wants to wrap up the day, or when `/end-of-day` reaches for it before drafting; `/to-plan` and `/do-plan` when `/execute-solution` reaches them to turn a picked solution into a real change; `coding-interview` when the user says "interview me"/"study mode"/pastes code for interview practice, without needing to name it. Most artifacts land in a skill-named directory at the project root; `/end-of-day`, `/d-handoff`, `/to-plan`, and `/experiment` instead read and write the global `~/wiki/` layout documented in `CLAUDE.md`'s Context Structure section (journal/research/handoff), since they're personal daily-workflow tools rather than per-project outputs.

The intent-to-cycle chain (`/intent-grill-me` → `/define-problem` → `/find-solutions` → `/execute-solution` → `/evaluate-solution`) runs in that order, each stage's output the next stage's input; `/evaluate-solution`'s Feedback recommends looping back into `/define-problem` or `/find-solutions` for any remaining gap, but never fires either automatically — that's always the human's call.
