...
| `/l1-implement` | — | Implement one L1 orchestration function directly from a plain-language use-case description, per `abstraction-levels.md`: decompose into L2/L3 (building each via `/l2-implement`/`/l3-implement`), reuse before creating, stub only what's underspecified, integration-test it — no plan file, lighter than `/dev-grill-me` |
| `/l2-implement` | — | Implement one L2 domain function directly from a plain-language business-rule description, via TDD, per `abstraction-levels.md`: depends on L3 only through an interface, never a concrete implementation |
| `/l3-implement` | — | Implement one L3 mechanism function directly from a plain-language technical-operation description, via TDD against the real mechanism, per `abstraction-levels.md`: exposes a simple interface upward, no business decisions inside |
