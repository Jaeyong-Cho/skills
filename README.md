...
| `/workflow` | `spec/**`, one review-plan per SEQ in `.context/inbox/plan/`, code changes | Run a goal straight through `/spec`'s scen → req → cmp → seq stages, fan out one subagent per resulting SEQ to run `/co-plan` scoped to just that sequence, then fan out one subagent per plan to run `/auto-action`'s Write & Test pass (haiku model) |
