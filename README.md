...
| `/loop-design` | — | Discuss the work-system topic, settle one smallest loop element, and hand a design brief to `/to-loop` |
| `/to-loop` | `loop/cycles/NN-slug/README.md` and needed scripts | Write the selected loop-design brief as one cycle definition, then hand it to `/loop-run` without executing it |
...
| `/verify-loop` | — | Use `/grill-me` to choose the cheapest sufficient target/domain test, then prepare, identify evidence-backed gaps, resolve safely, and stop on contradictory verification design |
...
| `/to-facts` | `contexts/NN-facts-{slug}.md` (or the confirmed wiki research topic) | Write this session's objective, background, key facts, current state, and open questions so a fresh session can resume it cold |
| `/to-intents` | `contexts/NN-intents-{slug}.md` (or the confirmed wiki research topic) | Extract the user's goals, desired outcome, motivation, constraints, boundaries, decisions, and open questions into a reusable intent document |
...
All skills above are user-invoked, except `/experiment`, `/d-handoff`, `/req`, `/ood`, `/to-plan`, `/do-plan`, `/follow-ai`, `/to-facts`, `/to-intents`, and `coding-interview`, which the agent can also fire on its own — `/experiment` when a question needs something actually run to get evidence; `/d-handoff` when the user wants to wrap up the day, or when `/end-of-day` reaches for it before drafting; the planning and implementation skills when their specific work is needed; `/follow-ai` when the user asks to learn or follow a topic step by step; `/to-facts` when a session's verified facts need recording; `/to-intents` when a session's user intent needs recording; `coding-interview` when the user says "interview me"/"study mode"/pastes code for interview practice, without needing to name it. `review-thermo` is dispatched only by `review-loop`. Project development artifacts use the target repository's conventions. `/end-of-day`, `/d-handoff`, and `/experiment` retain the global `~/wiki/` layout documented in `CLAUDE.md`'s Context Structure section.
