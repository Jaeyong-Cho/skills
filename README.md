...
| `/wbs` | `~/wiki/today/research/NN-{slug}/wbs/` | Break a big idea into an EPIC/STORY backlog — EPICs as business value, STORIES as the user-facing value delivered when done, no task-level breakdown — a draft only; `/to-plan` materializes a STORY into the target project's `spec/` when picked up |
...
| `/boy-scout` | — | Scan the diff since the last refactor commit (or branch base) through six lenses — architecture fit, interface depth, naming, clean code, simplicity, ambitious restructuring — report one small opportunistic cleanup worth doing, then `/grill-me` on value, behavior preservation, impact scope, testability; never edit |
