...
| `/next-way` | approved requirement, design, and plan artifacts; post-`/do-plan` result verdicts | Find the next actionable way; prepare `IMPL` through `/to-plan` without running `/do-plan`, then on a later invocation review its report against the expected result and way purpose before dependent work proceeds |
...
All skills above are user-invoked, except `/experiment`, `/d-handoff`, `/req`, `/ood`, `/to-plan`, `/do-plan`, `/follow-ai`, and `coding-interview`, which the agent can also fire on its own — `/experiment` when a question needs something actually run to get evidence; `/d-handoff` when the user wants to wrap up the day, or when `/end-of-day` reaches for it before drafting; `/req`, `/ood`, and `/to-plan` for an implementation handoff such as `/next-way`; `/do-plan` when an approved solution plan needs to be executed; `/follow-ai` when the user asks to learn or follow a topic step by step; `coding-interview` when the user says "interview me"/"study mode"/pastes code for interview practice, without needing to name it. Project development artifacts use the target repository's conventions. `/end-of-day`, `/d-handoff`, and `/experiment` retain the global `~/wiki/` layout documented in `CLAUDE.md`'s Context Structure section.
