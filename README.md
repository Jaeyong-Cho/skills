...
| `/wayfinder` | — | Plan concrete ways and tasks with consequential uncertainties, prerequisites, completion signals, and safe parallel execution; no implementation |
| `/to-way` | `ways/{nn}-{slug}/0-goal.md` and nested work-package files | Preserve a Wayfinder task plan, dependency links, and execution guidance; keep small tasks inline, with file numbers indicating presentation order only |
