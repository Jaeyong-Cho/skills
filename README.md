...
| `/experiment` | `~/wiki/journal/YYYY/MM/YYYY-MM-DD/research/NN-{slug}/experiments/{nn-{slug}.raw/,nn-{slug}/}` | Plan the cheapest method that would answer a question (via `/ponytail`), act on it for real, analyze the result into a verdict — supported, refuted, or inconclusive — then write it up as a `/to-paper` paper |
...
| `/to-paper` | `{slug}-research-paper/{index.html,manifest.json,assets/*.svg}` | Write a short HTML research paper — numbered Introduction/Background/Methodology/Results/Conclusion, at least 5 diagram-design-themed SVG figures — from a manifest.json, linted for writing-quality rules and built with a script; never hand-edit the HTML |
