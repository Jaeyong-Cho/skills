...
`mkdir -p .context/rdr` if needed. Tell the user the file path, and that it's a draft Requirement Decision Record that `/merge-req` will fold into `.context/req/{slug}.md` once implementation completes. If the written RDR runs over the template's word/sentence budget, say so and suggest running `/drilling` to split the goal further — but don't block on it. Next step: `/archi`.
