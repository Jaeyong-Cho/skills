...
- GTD: `~/wiki/gtd/{next-actions,waiting-for,someday-maybe,calendar,reference}.md`, `~/wiki/gtd/projects/{project-slug}.md` — persistent task system (David Allen's GTD); capture/clarify with `/gtd-grill-me`, file with `/to-gtd`, weekly-review with `/gtd-review`. Completed items/projects move to `~/wiki/gtd/archive/{yyyy}/{mm}/`.
