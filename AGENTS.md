...
- Goals: `~/wiki/goals/{slug}/` — persistent, multi-day research directory for an ongoing goal, tracked (active/done) in `~/wiki/goals.md`. Create with `ng <slug>` (`bin/ng`, on PATH; skills should call it, not write to `~/wiki/goals/` directly) — links it into `today/research/NN-{slug}/` immediately. List with `ng list` (shows an index); delete with `ng rm <slug|index>`. `/end-of-day` re-links every active goal into a fresh `today/research/NN-{slug}/` each day, and archives a goal marked `done` into `research/YYYY/MM/YYYY-MM-DD/NN-{slug}/` (that day's date) same as a one-off job.
- Index: `index.md` files under `~/wiki/`, `journal/`, `research/`, and `goals/` are nav chains (year -> month -> day); `/end-of-day` rebuilds the `research/` and `goals/` chains on each archive.
