...
- Format: every `~/wiki`/`spec` document carries OKF frontmatter (`type` required) per `references/document-style/frontmatter.md`; read frontmatter first, on every file, before the body.
