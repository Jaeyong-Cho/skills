...
name: pf-validate
description: |
  Scaffold, run, and report real sample validation for a feature — generates cases from grill/impl conclusions, runs each standalone, writes a lightweight HTML stats report (run_all.py) and a rich analysis HTML report (skill).
  Triggers: "pf-validate", "validate this feature", "add validation", "run validation", "sample test", or any request to validate implemented code with real data.
---
