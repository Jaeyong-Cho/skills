...
6. **Action sequence** — ordered atomic steps to implement. Each step: one concern, one logical unit, describable without "and". Always end the sequence with two fixed steps: write a changelog entry (`/to-changelog`), then remove the originating `.sot/TODO.md` item via `/to-todo` if one exists.
...
N+1. Remove the originating `.sot/TODO.md` item via `/to-todo`, if one exists
