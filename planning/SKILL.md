...
1. **Action sequence** — ordered atomic steps to implement, grounded in the ADR's Decision, Observability, Test-Loop Design, and Verification Criteria. Each step: one concern, one logical unit, describable without "and". The template's fixed Closeout checklist (refactor, test) always follows as the last steps — no need to grill those.

Grill until the sequence is resolved and the user confirms. Completion criterion: action sequence is fully ordered with no ambiguous steps, user confirmed.
