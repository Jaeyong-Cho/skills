...
6. **Action sequence** — ordered atomic steps to implement. Each step: one concern, one logical unit, describable without "and". Always end the sequence with three fixed steps: write domain truths or key decisions to the wiki via `/to-wiki`, write a changelog entry via `/to-changelog`, then remove the originating `.context/TODO.md` item via `/to-todo` if one exists.
...
...
N. Write domain truths or key decisions to the wiki via `/to-wiki`
