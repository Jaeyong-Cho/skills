...
6. **Action sequence** — ordered atomic steps to implement. Each step: one concern, one logical unit, describable without "and". After all implementation steps, always include these fixed steps in order:
   - **Refactor** — review changed code against meta-pattern and deep-module: readable code, readable architecture, clear naming, simple over clever, remove unused files and dead code, flatten unnecessary abstractions.
   - **Test** — run the test-loop to verify the refactoring didn't break anything.
   - Update `.context/wiki/` via `/to-wiki` if new truths were found or existing entries are stale (skip otherwise).
   - Write a changelog entry via `/to-changelog`.
   - Remove the originating `.context/TODO.md` item via `/to-todo` if one exists.
...
...
N. Refactor — readable code, readable architecture, clear naming, simple over clever, remove unused files and dead code, flatten unnecessary abstractions.
N+1. Test — run the test-loop to verify refactoring didn't break anything.
N+2. Update `.context/wiki/` via `/to-wiki` if new truths found or existing entries stale (skip otherwise)
N+3. Write a changelog entry via `/to-changelog`
N+4. Remove the originating `.context/TODO.md` item via `/to-todo`, if one exists
