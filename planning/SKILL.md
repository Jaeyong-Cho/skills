...
6. **Action sequence** — ordered atomic steps to implement. Each step: one concern, one logical unit, describable without "and". The template's fixed Closeout checklist (refactor, test, wiki, changelog, todo removal) always follows as the last steps — no need to grill those.
