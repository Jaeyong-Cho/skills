...
4. **Test-loop design** — first check if an existing test-loop already covers the needed behaviors; if so, reuse or extend it rather than creating a new one. Only design a new loop if the existing one cannot cover the scenarios. Apply `test-loop.md`: what is the clean state? what environment setup is needed? for each layer (UT / IT / E2E) — what scenarios does it exercise and what are the expected outputs? For E2E: what does `run` write to the output dir (results, version, input data, config) and what does `verify` check?
...
## Test-Loop Design
- **Clean state:** {what to reset and initialize before each run}
- **Environment setup:** {env vars, seed data, config}
- **UT:** {unit-level scenarios — isolated logic, minimal setup}
- **IT:** {integration scenarios — component interactions, real dependencies}
- **E2E:**
  - `run.py/run.sh`: {what it runs, what it writes to output-dir — results, metadata (version, input data, config)}
  - `verify.py/verify.sh`: {what it reads from output-dir and verifies}
