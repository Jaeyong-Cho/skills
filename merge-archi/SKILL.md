...
A draft ADR is named `{timestamp}-{slug}.md`; a merged one is `{timestamp}-{slug}.done.md` — the filename is the state, no in-file marker to parse, and the suffix matches how `/auto-action` marks a finished plan.
...
2. In `.context/adr/`, find the draft ADR file (no `.done.md` suffix) whose filename ends in `-{slug}.md`. If none, tell the user there's nothing to merge and stop. If more than one, list them and ask the user which to merge.
...
4. Rename the ADR file in place from `{timestamp}-{slug}.md` to `{timestamp}-{slug}.done.md` — keep it as permanent decision history, never delete it.
...
Completion criterion: the matching ADR file is renamed to `.done.md`, and `.context/archi/{slug}.md` reflects the implemented Static/Dynamic View.
