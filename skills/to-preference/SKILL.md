...
Read `../references/preference-format.md`, `../references/communication-style.md`, and `../references/document-style.md` first — together they define the standing-vs-one-off test, file locations, entry format, user-facing confirmation style, and persisted-bullet style that every step below uses.
...
3. **Confirm with the user** — list the candidates (rule, target file, new or flagged) using `../references/communication-style.md`: lead with the decision, then use concise bullets. This skill infers preferences after the fact rather than recording them live like `get-me` does, so nothing gets written without approval. Completion criterion: the user has approved a final set, possibly empty.
4. **Write** — append each approved candidate to its target file per `../references/preference-format.md`'s entry format and `../references/document-style.md`'s bullet rules, creating the topic file (and `.context/preferences/` directory) if it doesn't exist. Completion criterion: every approved candidate appears in its target file exactly once; no flagged duplicate was written twice.
