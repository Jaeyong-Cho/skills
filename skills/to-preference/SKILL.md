...
3. **Confirm with the user** — list the candidates (rule, target file, new or flagged) using `../references/communication-style.md`: lead with the decision, then use concise bullets. This skill infers preferences after the fact from the whole session, so nothing gets written without approval. Completion criterion: the user has approved a final set, possibly empty.
