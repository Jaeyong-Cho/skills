...
3. **Create each question's directory and grill it.** For every `## Question N` heading without a directory yet, slugify its text into `{slug}` (kebab-case, e.g. `cache-ttl-bound-p99-latency`) and create `questions/{slug}/`. Then, for that question and for any existing `questions/{slug}/` still missing `.context/grilling/` (a directory created before this convention, or backfilling after `/experiment`'s core stage stopped and asked for this), check for `questions/{slug}/.context/grilling/goal-context.md` first — `/question-brainstorm` propagates it there when it originated this question, per its own grilling pass over the goal. If present, use it as background instead of starting from nothing. Either way, **MUST RUN** `../grilling/SKILL.md` on the question itself, with the goal statement (and the propagated goal-context, if present) as background, to pin down the real intent, the real question (may not match the wording), non-negotiables vs. nice-to-haves, and known constraints. Save output to `questions/{slug}/.context/grilling/` — this is the canonical intent capture for the question; `/experiment`'s core stage reads it and never grills itself.
