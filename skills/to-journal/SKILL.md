...
3. **Pull the start time** — run `scripts/session_meta.sh` (relative to this skill's directory) to get the session's start time. It reads it from this session's own transcript; if none is found it reports `unknown` rather than guessing.
