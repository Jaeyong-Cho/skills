...
3. **Pull session metadata** — run `scripts/session_meta.sh` (relative to this skill's directory) to get the session's start time, model(s) used, skills invoked, token usage, and AIU cost (a dollar estimate from per-model Claude pricing on Claude Code; Copilot's premium-request billing unit elsewhere). It reads them from this session's own transcript; if none is found it reports `unknown`/`none` rather than guessing.
...
5. **Format the entry** as:
   ...
   - HH:MM:SS: AI {one-line title}
     ...
     - tokens: {TOKENS from session_meta.sh}
     - aiu: {AIU from session_meta.sh}
