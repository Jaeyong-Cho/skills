...
7. **Serve the gallery.** Copy `scripts/serve.sh` to the gallery directory to make run manually. (binds `0.0.0.0:4800`; pass a second arg to override the port), then report the URL to open: http://localhost:4800. Done when the server is running and the URL is stated.
