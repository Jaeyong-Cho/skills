...
4. **Leave one runnable check.** Every kind ends with something that fails if the build breaks, not just prose describing it: a skill's own self-test (the `--test` pattern `to-paper/scripts/lint_paper.py` uses), a CLI tool's `demo()`/`__main__` self-check or a `test_*` file, a custom agent's one real dispatch. No check written is not a done build.
