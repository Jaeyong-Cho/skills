...
2. **Follow the kind's own convention, don't invent one.** Deciding agent vs. skill in the first place → read `../references/subagents-vs-skills.md`.
   ...
   - **Custom agent** (a Leader/sub-agent definition) → use the active host's native agent mechanism. For Pi, read `../references/pi-custom-subagent.md`; detect the real installed platform by checking the tool itself (`--help`, its docs), never assumed from memory.
