...
7. **Draft the plan**, covering:
   ...
   - Action items, each as `- [ ] {item}` — the first item **MUST** be creating/checking out the git branch named in the grill-me session (`git checkout -b {branch}`), before any other action item runs. Also include one item that writes the Spec changes and Acceptance Criteria rows into the target project's `spec/{epic-slug}/{story-slug}.md` file(s) and updates both `spec/{epic-slug}/index.md` and the top-level `spec/index.md`, so the persisted spec stays synchronized with this plan. `@skills/do-plan` executes and checks these off in place.
   - Dogfood test
