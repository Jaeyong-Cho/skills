...
4. **Follow ELI5.** Make plan the so a fresh cheapest model agent can understand.
5. **Draft the plan**, covering:
...
6. **Write it** to `plans/{timestamp}-{slug}.md` (kebab-case slug of the plan's topic, timestamp from `date +%Y%m%d-%H%M%S`), creating the directory if needed.
7. **Split if too large.** If the file exceeds document-style's 500-word limit, split it into `plans/{timestamp}-{slug}-{n}-{part-slug}.md`, one file per vertical slice — each file a complete, independently executable unit with its own acceptance criteria and action items, not a horizontal layer (e.g. not "backend" + "frontend" for one feature).
