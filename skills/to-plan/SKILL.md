...
6. **Split if too large.** If the file exceeds document-style's 500-word limit, split it into `plans/{timestamp}-{slug}-{n}-{part-slug}.md`, one file per vertical slice — each file a complete, independently executable unit with its own acceptance criteria and action items, not a horizontal layer (e.g. not "backend" + "frontend" for one feature).
