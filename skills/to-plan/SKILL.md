...
7. **Draft the plan**, covering:
   ...
   - Spec changes — locate the target project's spec file(s) per `../references/spec-convention.md` (`spec/{epic-slug}.md`, one per EPIC; create `spec/` if it doesn't exist yet). List each affected requirement as its resulting final wording, one line each, grouped under its STORY — **MUST NOT** phrase it as a diff ("changed from X to Y", "renamed A to B"); the spec document only ever states the current state, never its history.
   ...
   - TODO.md items — one line per `TODO:`-tagged deferred item from the grill-me session (empty if none).
   ...
   - Release / Branch merge - **MUST CONFIRM** with the human before releasing or merging any branch.
   ...
   - Action items, each as `- [ ] {item}` — the first item **MUST** be creating/checking out the git branch named in the grill-me session (`git checkout -b {branch}`), before any other action item runs. Also include one item that writes the Spec changes and Acceptance Criteria rows into the target project's `spec/{epic-slug}/{story-slug}.md` file(s) and updates both `spec/{epic-slug}/index.md` and the top-level `spec/index.md`, so the persisted spec stays synchronized with this plan. If the TODO.md items list above is non-empty, also include one item that appends them to the target project's TODO.md `## Later` section (per `@skills/project-setup`; create the file if missing). `@skills/do-plan` executes and checks these off in place.
   - Dogfood test
