...
7. **Draft the plan**, covering:
   ...
   - Action items, each as `- [ ] {item}` — the first item **MUST** be creating/checking out the git branch named in the grill-me session (`git checkout -b {branch}`), before any other action item runs. If the TODO.md items list above is non-empty, also include one item that appends them to the target project's TODO.md `## Later` section (per `@skills/project-setup`; create the file if missing). `@skills/do-plan` executes and checks these off in place.
   - Dogfood test
