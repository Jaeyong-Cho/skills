...
8. **Draft the plan**, covering:
   ...
   - Dogfood test
9. **Write it.** Default to `./plans/{nn}-{slug}.md` in the current directory, creating `plans/` if needed, `{nn}` the next zero-padded sequence number inside it (count existing files there; starts at `01`) — unless the user asks to file it under the wiki instead, in which case read `../references/research-topic-directory.md` first, confirm `{NN}-{slug}` per `../references/question-format.md`'s ❓/➡️ format (skip re-asking if already confirmed earlier this session), then write to `{NN}-{slug}/plans/{nn}-{slug}.md` under `~/wiki/today/research/`.
