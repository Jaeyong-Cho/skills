...
6. **Follow todo convention.** Read `../references/todo-convention.md` first, then write one `todo/{topic-slug}.md` per vertical slice (per sub-topic in step 4) using `../template/todo.md`'s format exactly: a link back to its spec sub-topic, then `- [ ] action` items that build the slice end-to-end (not layered by frontend/backend). Pull items only from this session's settled decisions. It's temp scratch, delete once checked off.
7. **Update `todo/index.md`.** Per `../references/todo-convention.md`'s WBS guide format, add every new slice into its round based on this session's settled slice ordering, marking the critical path — an unlisted slice is treated as not scheduled.
...
Completion criterion: every topic from this session has its own spec file with every sub-topic's AC block traceable to a decision from this session, plus its own `todo/{topic-slug}.md` scheduled in `todo/index.md`, and `spec/index.md`/`spec/overview.md` reflect the same change.
