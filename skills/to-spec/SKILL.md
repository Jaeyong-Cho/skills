...
Turn this session's decisions into the target project's persisted spec, plus a temp todo scratch file per slice. Unlike `@skills/to-plan`, this writes no branch/release plan; use it when the spec itself is the deliverable (e.g. spec-first, or documenting settled behavior) rather than a step toward `@skills/do-plan`.
...
6. **Follow todo convention.** Read `../references/todo-convention.md` first, then write one `todo/{topic-slug}.md` per vertical slice (per sub-topic in step 4) using `../template/todo.md`'s format exactly: a link back to its spec sub-topic, then `- [ ] action` items that build the slice end-to-end (not layered by frontend/backend). Pull items only from this session's settled decisions. No index, no ceremony — it's temp scratch, delete once checked off.
7. **Update the project overview.** Update `spec/overview.md` (create it if missing, per `../references/spec-convention.md`) so its paragraph still describes what the project does after this change, linking to `spec/index.md` for detail. Don't restate requirements or AC there — link, don't duplicate.

Completion criterion: every topic from this session has its own spec file with every sub-topic's AC block traceable to a decision from this session, plus its own `todo/{topic-slug}.md`, and `spec/index.md`/`spec/overview.md` reflect the same change.
