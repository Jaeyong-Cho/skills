...
**MUST** surface assertions aggressively wherever there is any uncertainty — for each, name the agent/skill/file it belongs to and whether it's a precondition, invariant, or postcondition; this skill writes each one as a real runtime assert directly in that exact file, at the exact point it checks, not a comment and not deferred to a doc.
