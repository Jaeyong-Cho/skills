...
3. **Draft in document style** — using `../references/document-style.md`, write: an Introduction (objective, background, scope, methodology), a Body (facts, analysis, findings, evidence), and a Conclusion (key takeaways, recommendations, next actions). Short sentences, one idea each, key information first. Before defaulting to bullets, check each chunk of Body content against the priority order: attributes/config/facts about a single subject → key-value block; options weighed, before/after, or other parallel-attribute content → Markdown table; a sequence of steps/stages the session went through → ASCII flow diagram, only if a diagram genuinely reads faster than the tiers above; only content with no flow, comparison, or single-subject-attribute shape stays as bullets.
