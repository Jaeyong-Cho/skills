...
1. **Gather today's sources** — read today's journal file (`~/wiki/journal/YYYY/MM/YYYY-MM-DD.md`) in full and list today's research directories (`~/wiki/research/YYYY/MM/YYYY-MM-DD/NN-{job}/`). If neither exists, tell the user and stop. Completion criterion: every journal entry and every research directory for today is either read in full or confirmed absent.
