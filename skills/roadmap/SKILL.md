...
1. **State is location.** Which of `open/`, `in-progress/`, `done/` an EPIC's directory sits under *is* its state — moving it (`mv`) is the state change, never a separate status field to drift out of sync. STORIES nest inside their EPIC (`{epic-slug}/{story-slug}/`) and move with it.
...
3. **Run grill-me open-ended.** `@skills/grill-me` over whatever came up, grouped CRUD-style. Fix is an Update, not its own category — a misnamed, mis-scoped, or misplaced item gets corrected in place, same as a deadline edit.
   - **Create**
     ...
     - New EPIC — settle its name, goal, deadline, and STORIES (each with a goal, an optional own deadline that inherits the EPIC's when unset, and its first Tasks). Write `open/{epic-slug}/index.md` from `../template/roadmap-epic-index.md`, and `open/{epic-slug}/{story-slug}/index.md` per STORY from `../template/roadmap-story-index.md`.
   ...
   - **Update**
     ...
     - Fix — rename a misnamed item (its directory + `#` heading), re-parent a STORY to a different EPIC (`mv {epic-a}/{story-slug}/ {epic-b}/{story-slug}/`), or split/merge EPICs (`mkdir`/`mv`/`rmdir`).
   ...
   - **Delete** — `rm -r` the EPIC/STORY/Task outright. Reserved for cancelled or no-longer-relevant work; finished work is an Update (`mv` to `done/`), never a delete.
