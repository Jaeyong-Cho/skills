...
3. **Run grill-me open-ended.** `@skills/grill-me` over whatever came up, grouped CRUD-style. Fix is an Update, not its own category — a misnamed, mis-scoped, or misplaced item gets corrected in place, same as a deadline edit.
   - **Create**
     ...
     - New EPIC — settle its name, goal, deadline, and STORIES (each with a goal, an optional own deadline that inherits the EPIC's when unset, and its first Tasks). Write `{project}/open/{epic-slug}/index.md` from `../template/roadmap-epic-index.md`, and `{project}/open/{epic-slug}/{story-slug}.md` per STORY from `../template/roadmap-story.md`.
   ...
   - **Update**
     ...
     - State — `mv` the EPIC's whole directory between `{project}/open/`, `{project}/in-progress/`, `{project}/done/`; its STORIES move with it.
