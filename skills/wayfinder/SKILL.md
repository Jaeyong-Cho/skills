...
An `IMPL` task owns the implementation work for its product-code scope. Its How describes this order: implement the scoped change; run the relevant REQUIRED checks and review the implementation evidence against the task's expected result state and the way's purpose; then commit the resulting change and record its commit hash/evidence. An `IMPL` task is not `done` until the evidence review is complete, REQUIRED verification passes, and the commit hash/evidence is recorded. It may be `ready` when available context is sufficient, with missing context recorded as a risk. Do not add separate lifecycle nodes merely to represent these process steps. A critical requirement or result contradiction should be reported and may be resolved by revising the existing way.
...
- **Never append a fixed process sequence to each leaf.** Add a separate task only when a distinct decision, deliverable, integration, regression, or release check is needed. Put an IMPL task's implementation, relevant REQUIRED checks/review, and post-review commit evidence in that task's completion signal. Product-code implementation is represented by an inline `Kind: IMPL` task at the relevant outcome, not by a lifecycle chain beneath every leaf.
...
- **How:** the concrete approach and steps needed to produce that result, including what to inspect/reuse/change and how to check it. Use a short ordered list when sequence matters; a single specific action suffices for a trivial task. A generic process list is not an approach. An `IMPL` task ends, in order, with implementation evidence reviewed against `What`, `Done when`, and the parent purpose, REQUIRED verification passing, and the resulting change committed with its commit hash/evidence recorded.
...
- Use `ready`, `waiting` (unfinished prerequisites), `blocked` (unresolved uncertainty/decision), or `done` (evidence supplied). These statuses guide sequencing rather than create hard gates; the user may choose to proceed from `waiting` or `blocked` work after the risk is stated. An `IMPL` task is normally considered fully verified only after its implementation evidence supports the expected result state and parent purpose, the review is recorded, REQUIRED verification passes, and the resulting change is committed with commit hash/evidence recorded; if any of that evidence is missing, label the result unverified and state the risk rather than silently claiming success. Parent completion means its required children are complete; depend on the relevant leaves, not on parents or descendants of yourself.
