#!/usr/bin/env python3
"""
render.py — render a structured study-guide spec into the self-contained HTML
page format used by the `study-guide` skill.

Why this exists: the CSS, quiz JavaScript, and page scaffolding are identical
across every invocation of the skill — only the content (prose, diagrams,
quiz questions) actually changes per document or codebase. Regenerating that
...
If -o is omitted, writes to <dir-of-spec>/YYYY-MM-DD-<slug>.html, where
<slug> comes from the spec's "slug" field.

Spec format (JSON):
{
  "title": "Understanding the retry logic: exponential backoff with jitter",
  "subtitle": "Study guide · src/retry.py",
  "slug": "retry-backoff",
  "sections": [
    {"id": "background", "heading": "Background", "html": "<p>...</p>"},
    {"id": "concepts", "heading": "Core concepts", "html": "<p>...</p><div class=\"diagram\">...</div>"},
    {"id": "walkthrough", "heading": "Walkthrough", "html": "<pre><code>...</code></pre>"}
  ],
  "quiz": [
    {
...
tables, ".diagram"/".callout" divs per the classes below), not markdown. This
keeps the script a pure template renderer; all the writing judgment (what to
explain, which diagrams to draw, which examples to use) still belongs to the
LLM following the study-guide skill — this just removes the repetitive part.
"""
import argparse
import datetime
import html
...
CSS = """
  ...
  .example { background: #f4f8f4; border-left: 4px solid #2f7d4f; padding: .9rem 1.2rem;
    border-radius: 0 6px 6px 0; margin: 1.2rem 0; }
...
def main():
    ...
    if args.output:
        ...
        out_path = args.spec.parent / f"{date_prefix}-study-guide-{slug}.html"
