...
Before asking each round's frontier, check `~/wiki/kb` for an existing answer instead of asking something already settled: run `python3 ../to-kb/scripts/kb.py search {keyword} [{keyword} ...]` (relative to this skill's directory) with that question's topic words — it matches every OKF descriptive field (`type`/`title`/`description`/`tags`), not tags alone, plus the doc's domain/category path. A confident top match (score clearly ahead of the rest) means: skip the question, don't ask it — state the answer and cite the doc path, marked 📚 so it stays visible/auditable in the transcript. No confident match: ask the question as usual. This step only *reads* the KB — never edit its frontmatter here; `@skills/to-kb` is the only thing that updates `hit_count`/`last_hit_at`, and only after a finished session confirms the answer actually held up.
