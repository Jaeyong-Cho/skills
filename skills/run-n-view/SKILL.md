---
name: run-n-view
description: Run something for real, then build a @skills/viewpoints gallery over its output — launch/drive a command, script, test suite, or app via the run skill, capture the real output, and hand it to @skills/viewpoints for a gallery of charts/diagrams instead of raw text. Use when invoked as /run-n-view.
...
3. **Build the gallery in a subagent.** Dispatch a subagent (Agent tool, claude-sonnet-5 model, `run_in_background: false`) pointed at `run-n-view/{slug}/raw/` with the instruction: "with @skills/viewpoints, build a gallery over <describe the raw output>, output to `run-n-view/{slug}/gallery/`." Isolating this keeps viewpoints' own profiling/shortlisting/rendering legwork out of the main run; `run_in_background: false` is required because step 4 reads the gallery's output. Done when `run-n-view/{slug}/gallery/index.html` exists on disk — confirm with a file check, not the subagent's summary alone.
