...
2. **Select checklists and fact-find before grilling.** Read the applicable checklist references for the topic. Always use the constraint discovery checklist in `@skills/to-constraint` when collecting constraints, including coding style and repository conventions plus how to build, test, release, and deploy. Apply each relevant point as a coverage check; record only evidence, and mark missing categories as **Unresolved questions** or **Not established** rather than inventing them. Before starting either grill, identify every initial environment question that can be answered by inspection or an isolated trial and dispatch a sub-agent for each one with a focused question, explicit scope/workdir, expected evidence, and safety boundary. `EXPLORE` sub-agents are strictly read-only; `EXPERIMENT` sub-agents must first read and invoke/follow `@skills/experiment` inside their own work, explicitly using its required **plan → act → analyze** evidence flow, working in isolation, and making no production-state changes. Use `EXPLORE` for read-only inspection or searching existing artifacts, and use `EXPERIMENT` only when exploration cannot answer a consequential question. Review every returned sub-agent result against its expected evidence before using it. Record supported results as **Verified facts**, unsupported or inconclusive results as **Unresolved questions**, and never treat an unreturned or unsupported result as fact. Do not ask the user for environment facts the AI can inspect.

3. **Clarify with `@skills/grill-ai` after initial fact-finding.** Start grill-ai only after the initial `EXPLORE` and `EXPERIMENT` dispatches have returned and been reviewed. Give it the current buckets and evidence so it can identify understanding gaps, distinguish facts from assumptions, and decide which remaining unknowns the AI can verify. If it identifies another needed investigation, dispatch it before starting grill-me and review its result using the same evidence rules. Grill-ai handles clarification and fact-finding strategy; it does not implement production changes.

4. **Resolve user-owned decisions with `@skills/grill-me` after fact-finding.** Start grill-me only after all investigations identified so far have returned and been reviewed. Give it the current buckets, verified findings, applicable checklists, and remaining decision frontier. It must ask the user the focused, round-by-round questions needed to close understanding gaps and drive decisions. Keep user choices in **Intents**, **Constraints**, or **decisions within Verified facts** as appropriate. If the user accepts a recommendation without deciding, record it as an **Assumption**, not a verified fact. Continue until the frontier is empty, then show the separated summary and ask the user to confirm shared understanding. Do not persist before confirmation.

5. **Reconcile before capture.** Ensure every material item belongs to exactly the appropriate bucket and that inferred or provisional items remain labeled as assumptions. Preserve unresolved questions rather than silently answering them. If new evidence or user answers change an earlier item, update the working record and reconfirm the summary.
