...
2. **Execute each unchecked action item, in order.** For each: implement it for real, verify the result, then edit the plan file to check it off (`- [x]`) before moving to the next. **MUST**, when an action item touches a function named in the plan's Assertions section, write that line's assert as a real runtime statement in the implementation code at the exact point it checks (language-native `assert`/equivalent, not a comment, not test-only) — same rigor as Verification Method rows, uncertainty in the code gets a real assert, not a TODO. If the item writes or updates the target project's spec, follow `../references/spec-convention.md` (`spec/{epic-slug}/{story-slug}.md` per `../template/spec.md`, its `spec/{epic-slug}/index.md` and the top-level `spec/index.md` kept in sync). Stop and ask the user if an item is ambiguous or its prerequisite failed — don't skip it. Completion criterion: every action item in the plan is either `- [x]` or explicitly called out as blocked. **MUST CHECK** the spec document has acceptance criteria)
