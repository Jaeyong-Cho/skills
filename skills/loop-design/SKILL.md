...
Invoking this skill enters **Design Loop Mode**: think about the smallest executable cycle for building or setting up a work system. This mode designs the feedback loop, not the product work itself. **MUST RUN** `@skills/grill-me` for share the understanding with user's goal, intent, repository state, and run evidence to define the cycle.
