...
3. **Write the L3 function.** Technical mechanism only — no business decision beyond what the mechanism itself strictly requires (a retry/timeout policy is fine, a discount rule is not). Hide the SDK/HTTP/DB details behind the interface from step 1, per `../references/abstraction-levels.md`'s L3 rules and rule 6 (interfaces define contracts, not mechanisms). Name it per `../references/naming.md`.
