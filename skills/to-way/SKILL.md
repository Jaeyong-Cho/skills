...
1. **Gather and validate before writing.** Use the supplied plan or the complete Wayfinder result in the session, including its work map and execution notes. If missing, ask for it. Check:
   ...
   - every `done` task records completion evidence when available; an `IMPL` task should include a review of its implementation evidence against the expected result state and parent purpose when available. Never invent evidence from its title, status, green build, or commit alone.

   Explicitly partial branches blocked by named uncertainties are valid; retain their limits. Missing planning information is not permission to invent it. For a legacy requirements/design/implement/test tree or other incomplete source, explain the missing decisions and preserve the incomplete state rather than silently upgrading it or serializing empty placeholders.
...
4. **Record progress explicitly.** Preserve an existing `Status: in-progress` marker during updates. When work starts, record `Status: in-progress` when practical; when the source supplies evidence that a task finished, write `Status: done` and retain the evidence beside that status. Keep not-started tasks `ready`, `waiting`, or `blocked`; do not infer completion because a dependent task is done. If a task is marked done without evidence, preserve the status but label the evidence as missing or unverified and report the gap.
...
- Evidence: <proof for a done status when available; for IMPL include a review of implementation evidence against `What`, `Done when`, and parent purpose when available; identify missing or unverified evidence explicitly>
...
Do not infer new prerequisites or parallel lanes from file numbers. If a needed field is missing rather than explicitly unknown/blocked, report the gap rather than inserting `Planned` or `None` as filler.

## Completion criterion

Every source node has exactly one canonical location; every task records its resolved absolute Workdir and every `IMPL` task records its resolved absolute Target repo; every way preserves its purpose, evidence-grounded current state, expected result state, hypothesis, and assumptions; every task preserves its current state and expected result state; all planning meaning survives recording, every link resolves, and a reader can start at `0-goal.md` to find what can proceed, what may wait and why, how to resolve blockers, and what proves completion. Every leaf retains one of `EXPLORE`, `EXPERIMENT`, `IMPL`, or `CHECKOUT`; `IMPL` leaves remain inline with their implementation approach and review evidence preserved when available. New goals are created without collisions, existing goals are updated in place, task workspaces are created only when work or artifacts require them, annotations and in-progress/completed tasks are preserved, and missing evidence is reported rather than hidden. No redundant task files, lost dependency edges, invented work, or unapproved migration remains.
