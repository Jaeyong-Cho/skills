...
2. **Grill the user.** Dispatch a subagent (Agent tool) with claude-sonnet-5 model to run the `grilling` skill on the user's request, giving it step 1's evidence file as background, focused on three things: the real intent, the real question being tested (may not match the literal wording), and why it matters enough to spend an experiment on. Pass `run_in_background: false` so the interview completes before proceeding. And make sure the subagent's output is saved to `experiments/{slug}/.context/grilling/` so it can be referenced in step 3.
