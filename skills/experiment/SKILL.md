...
**MUST Write** the plan, raw output, and verdict under `~/wiki/research/{date}/{NN}-{slug}/`, in `../references/document-style.md` style — `{date}` from `date +%Y/%m/%Y-%m-%d`, `{slug}` a kebab-case slug of the question, `{NN}` the next zero-padded sequence number for that day (count existing `NN-*` directories under the day's folder; reuse the same `{NN}-{slug}` directory as `/explore` when escalated from it).
