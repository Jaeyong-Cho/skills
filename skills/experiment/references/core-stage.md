...
**Read prior grilling — every attempt.** `/goal-init` already grilled this question when its directory was created; read `questions/{slug}/.context/grilling/` for the real intent, the real question (may not match the wording), and why it's worth an experiment — treat it as sufficient, don't re-grill. Grilling happens only at question-making time (`/goal-init` or `/question-brainstorm`), never here — the orchestrating thread runs on a more expensive model, and grilling is exactly the interview work that belongs upstream. No `.context/grilling/` present (question predates this convention): stop and tell the user to run `/goal-init` on this question to backfill it, don't grill inline. Re-grill only if the next-attempt gate below decided a prior attempt's angle was based on a wrong premise about intent — and even then, by re-running `/goal-init` on this question, not inline here.
...
**Analyze against the hypothesis.** **MUST DISPATCH** a claude-sonnet-5 subagent (Agent tool), briefed with this attempt's `hypothesis.md` and its raw results (`raw/output.md` or `raw/group-*/`), to compare them against the confirming/refuting observations and write `experiments/{n}-{angle-slug}/result.md` directly: angle, tier, verdict — supported, refuted, or inconclusive (state why if inconclusive) — the reasoning, and, if inconclusive or superseded by a later attempt, what a next attempt would need to cover. `run_in_background: false`. Read back only the verdict and a one-line reasoning summary before applying the next-attempt gate below.
