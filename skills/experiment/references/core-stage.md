...
**Read prior grilling — every attempt.** `/goal-init` already grilled this question when its directory was created; read `questions/{slug}/.context/grilling/` for the real intent, the real question (may not match the wording), and why it's worth an experiment — treat it as sufficient, don't re-grill. No `.context/grilling/` present (question predates this convention) — grill it now yourself, same scope, and save it there so later attempts don't repeat this. Re-grill only if the next-attempt gate below decided a prior attempt's angle was based on a wrong premise about intent, not just an execution gap.
