...
2. **Ask which research topics stay open** — list today's `~/wiki/today/research/NN-{job}/` dirs and ask the user which are done (archive) vs. still in progress (remain under `today/` for tomorrow). Default to archiving all if the user has no preference. Completion criterion: every research dir has an explicit archive/remain decision.
3. **Archive today's working files** — run `bash scripts/archive_today.sh` (relative to this skill's directory), passing `--keep "NN-job,NN-job"` with the remain list from step 2 (omit `--keep` if none). It moves `~/wiki/today/journal.md` into `~/wiki/journal/YYYY/MM/YYYY-MM-DD/journal.md` and each non-kept `~/wiki/today/research/NN-{job}/` entry into `~/wiki/journal/YYYY/MM/YYYY-MM-DD/research/NN-{job}/`; kept entries stay under `~/wiki/today/research/` untouched. The date used is `today/journal.md`'s creation date, not the date the script happens to run on — a session that starts in the evening and is archived after midnight still files under the day it started. Run this after step 1, not before — d-handoff still needs `today/` in place. Completion criterion: the script ran, `~/wiki/today/journal.md` no longer exists, and every non-kept research dir moved.
4. **Seed tomorrow's work context** — write a fresh `~/wiki/today/journal.md` (`mkdir -p` the parent; step 3 just removed this file, so create it new — don't reuse an old one; any kept research dirs from step 2 are already sitting under `~/wiki/today/research/`), with one section:
...
5. **Gather today's sources** — read today's journal file (`~/wiki/journal/YYYY/MM/YYYY-MM-DD/journal.md`), archived research directories (`~/wiki/journal/YYYY/MM/YYYY-MM-DD/research/NN-{job}/`), any kept research directories still under `~/wiki/today/research/NN-{job}/` from step 2, and the file from step 1, all in full. Completion criterion: every source is either read in full or confirmed absent.
6. **Split into content items** — one item per topic, task, or research job from the journal/research, plus one "Handoff" item from the handoff file. Note in each kept-topic item that it remains open and continues under `~/wiki/today/research/`. Completion criterion: every entry from step 5, including the handoff file, is assigned to exactly one content item.
