...
4. **Gather today's sources** — read today's journal file (`~/wiki/journal/YYYY/MM/YYYY-MM-DD/journal.md`), today's research directories (`~/wiki/journal/YYYY/MM/YYYY-MM-DD/research/NN-{job}/`), and the file from step 1, all in full. Completion criterion: every source is either read in full or confirmed absent.
