...
2. **Test.** Run the plan's test scope: read the plan and ADR/RDR for test strategy and scope (default to both unit and integration unless the plan says otherwise), then run those tests and record pass/fail per test.
