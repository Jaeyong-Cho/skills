...
Completion criterion: the L1 function exists and calls only named L2/L3 functions, no inline mechanism; every gap from step 2 is an explicit loud TODO stub naming the missing decision and which skill builds it for real; the integration test ran and its actual result is in hand.

Tell the human the function's file:line, which L2/L3 functions were reused vs. stubbed (and which skill each stub needs), and the test result, when done.
