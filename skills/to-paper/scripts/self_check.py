...
Vendored verbatim from ../diagram-design/scripts/self_check.py (lint_paper.py
imports this copy directly, so to-paper's diagram checks don't reach into a
sibling skill's directory) — re-sync by copying that file over this one if
diagram-design's checks change. Ships inside the skill so an installed agent
...
# Points at diagram-design's own assets (not a local copy) even when this
# file is vendored elsewhere — the canonical motion controller has one home.
MOTION_TEMPLATE = Path(__file__).resolve().parent.parent.parent / "diagram-design" / "assets" / "template-motion.html"
