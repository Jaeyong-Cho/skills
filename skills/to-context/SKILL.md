...
4. **Write it.** If this session already wrote a `contexts/{nn}-{slug}.md` file, update that file in place with the current Objective/Background/Key facts/Current state/Open questions instead of creating another one — a context document tracks the session's latest state, not a history of drafts. Otherwise, **MUST ASK** confirmation of the directory first, per `../references/question-format.md`'s ❓/➡️ format — recommend the current directory (`./contexts/`) as the default, unless the user asks to file it under the wiki instead, in which case read `../references/research-topic-directory.md` first and confirm `{NN}-{slug}` the same way; skip re-asking if already confirmed earlier this session. Once confirmed, write to `./contexts/{nn}-{slug}.md` (creating `contexts/` if needed) or `{NN}-{slug}/contexts/{nn}-{slug}.md` under `~/wiki/today/research/`, whichever was confirmed — `{nn}` the next zero-padded sequence number inside `contexts/` (count existing files there; starts at `01`).
