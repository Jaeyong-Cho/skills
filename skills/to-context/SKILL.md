...
4. **Write it.** If this session already wrote a `contexts/{nn}-{slug}.md` file, update that file in place with the current Objective/Background/Key facts/Current state/Open questions instead of creating another one — a context document tracks the session's latest state, not a history of drafts. Otherwise, default to writing at `./contexts/{nn}-{slug}.md` in the current directory, creating `contexts/` if needed, `{nn}` the next zero-padded sequence number inside it (count existing files there; starts at `01`) — unless the user asks to file it under the wiki instead, in which case read `../references/research-topic-directory.md` first and confirm `{NN}-{slug}` per `../references/question-format.md`'s ❓/➡️ format, then write to `{NN}-{slug}/contexts/{nn}-{slug}.md` under `~/wiki/today/research/`.
