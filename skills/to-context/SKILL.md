...
4. **Write it.** If this session already wrote a `contexts/{nn}-{slug}.md` file, update that file in place with the current Objective/Background/Key facts/Current state/Open questions instead of creating another one — a context document tracks the session's latest state, not a history of drafts. Otherwise write to `~/wiki/research/{date}/{NN}-{slug}/contexts/{nn}-{slug}.md`, creating the directory if needed — `{date}` from `date +%Y/%m/%Y-%m-%d`, `{slug}` a kebab-case slug of the session's topic, `{NN}` the zero-padded sequence number for that day, starting at `00` (count existing `NN-*` directories under the day's folder; reuse the same `{NN}-{slug}` directory as `/explore` or `/experiment` if this session continues one of those jobs), `{nn}` the next zero-padded sequence number inside `contexts/` (count existing files there; starts at `01`).
