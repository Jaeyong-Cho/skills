...
After deriving SRS items, check whether the project already has a debugger CuRS:
...
### Create CuRS-{NNN}: Runtime Observability via CLI Debug Options
...
### Create SRS-{NNN}: Debug Level Control
...
### Create SRS-{NNN+1}: Debug Output Directory, Log Format, and Data File Rules
...
### Create AT-{NNN}: Debug CLI Options Work at Runtime
...
Add all new items to `SUMMARY.md` and their respective index files. Update `tags.md` with the `#debugger` tag if not already present.
